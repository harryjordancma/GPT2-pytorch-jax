...
class LocalCausalSelfAttention(nn.Module):
    ...
    def _create_combined_mask(self, block_size, window_size):
        """
        Create a combined mask that enforces both causal and local attention.
        """
        # Create a causal mask (lower triangular)
        causal_mask = torch.tril(torch.ones(block_size, block_size))  # (T, T)

        # Create a local mask
        local_mask = torch.ones(block_size, block_size)  # (T, T)
        for i in range(block_size):
            start = max(0, i - window_size + 1)
            local_mask[i, :start] = 0  # Mask tokens before the window

        # Combine the masks
        combined_mask = causal_mask * local_mask  # (T, T)

        # Reshape for multi-head attention
        combined_mask = combined_mask.view(1, 1, block_size, block_size)  # (1, 1, T, T)
...
class TinyStoriesConfig:
    ...
    # One layer head for tiny stories
    n_layer: int = 1
    n_head: int = 12
    n_embd: int = 768
    # for local attention
